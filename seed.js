...
async function main() {
  ...
  const scenarioNames = Object.keys(scenariosObj);
  console.log(`Seeding ${scenarioNames.length} scenarios...`);
  for (const name of scenarioNames) {
    const s = scenariosObj[name];
    const doc = {
      name,
      summary: s.summary || '',
      steps: s.steps || [],
    ...
    await Scenario.findOneAndUpdate({ name }, doc, { upsert: true });
  }
